...
webpush.setVapidDetails(
  "mailto:alejandro.martinez.22s@utzmg.edu.mx",
  publicVapidKey,
  privateVapidKey
);
...
router.post("/suscribir", async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({
      ok: true,
      msg: "Usuario suscrito correctamente",
      user,
    });
  } catch (err) {
    console.error(err);
...
router.post("/notificar/:id", async (req, res) => {
  ...
  try {
    ...
    if (!usuario || !usuario.puedeRecibir || !usuario.subscriptionToken) {
      return res
        .status(400)
        .json({ msg: "El usuario no puede recibir notificaciones" });
    }
